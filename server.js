...
async function handleEvent(event) {
  ...
  if (doneMatch) {
    ...
    return await client.replyMessage(event.replyToken, { type: 'text', text: `${name}已標記完成！` });
  }
  ...
  if (noteMatch) {
    ...
    return await client.replyMessage(event.replyToken, { type: 'text', text: `已記錄備註：${name}-${note}` });
  }
  ...
  if (queryMatch) {
    ...
    if (!tasks[dateStr]) {
      return await client.replyMessage(event.replyToken, { type: 'text', text: `${dateStr} 尚無資料` });
    }
    ...
    return await client.replyMessage(event.replyToken, { type: 'text', text: msg });
  }
  ...
  if (text === '推播') {
    ...
    if (!userId) {
      return await client.replyMessage(event.replyToken, { type: 'text', text: '推播只能在個人聊天使用！' });
    }
    await pushTodayTask(userId);
    return await client.replyMessage(event.replyToken, { type: 'text', text: '已推播今日任務按鈕！' });
  }

  // echo
  return await client.replyMessage(event.replyToken, { type: 'text', text: `你說的是：${text}` });
}
